...
namespace Yarukizero.Net.MakiMoki.Shared.Bindings {
	/*
	 * NativeCommandはDisposeされていると例外吐いて制御が大変なので例外無視するラッパクラス
	 * 
	 */

	class MakiMokiCommand<T> : IObservable<T>, ICommand, IDisposable {
	...
	class MakiMokiCommand : MakiMokiCommand<object> {
